...
export default function EditableDebtorProfileForm({ initialProfile }: { initialProfile: any }) {
  ...
  useEffect(() => {
    if (!initialProfile && walletAddress && !profile) {
      const fetchProfile = async () => {
        setIsLoading(true);
        try {
          const response = await fetch(`/api/verification/debtor/profile?walletAddress=${walletAddress}`);
          const data = await response.json();
          if (data.success) {
            setProfile(data.profile);
          }
        } catch (error) {
          console.error("Failed to fetch debtor profile:", error);
        } finally {
          setIsLoading(false);
        }
      };
  ...
  const handleSubmitVerification = async () => {
    ...
    try {
      const response = await fetch(`/api/verification/debtor/submit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          walletAddress: profile.walletAddress,
  ...
  if (!profile) {
    // No profile exists yet - show call to action for new user to start verification
    const emptyProfile = {
        ...
        <div className="p-4 rounded-xl border border-rose-400/30 bg-rose-400/5">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h4 className="font-semibold text-rose-400">Action Required: Start Verification</h4>
              <p className="mt-1 text-sm text-rose-400/80">
                Create your debtor profile to acknowledge and repay receivable Offers.
              </p>
              <button
                onClick={() => {
      ...
      <div className="flex items-start gap-6">
        <div className="flex-1">
          {isEditing ? (
            ...
            <>
              <h3 className="text-xl font-semibold text-white">{profile.name || "Debtor Profile"}</h3>
