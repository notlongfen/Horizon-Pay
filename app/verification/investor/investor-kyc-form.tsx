...
import { saveInvestorDraft } from "@/lib/verification/verification-service";
import { BorderGlow } from "@/app/components/border-glow";
import { Card, SectionLabel, ArrowGlyph } from "@/app/components/ui";

const investorInfoFields = [
  {
    name: "name",
    label: "Full Name / Institution Name",
    type: "text",
    placeholder: "Enter your name or institution name",
    required: true,
  },
  {
    name: "investorType",
    label: "Investor Type",
    type: "text",
    placeholder: "e.g., INDIVIDUAL, INSTITUTIONAL, CORPORATE",
    required: true,
  },
  {
    ...
    type: "text",
    placeholder: "If institutional investor, enter institution name",
    required: false,
  },
  {
    name: "email",
  ...
  },
  {
    ...
    type: "text",
    placeholder: "e.g., ACCREDITED, NON_ACCREDITED, PENDING",
    required: false,
  },
  {
    name: "accreditationDocument",
    label: "Accreditation Document Reference",
    type: "text",
    placeholder: "Reference to accreditation document",
    required: false,
  },
] as const;

const investorDocumentTypes = [
  {
    type: "GOVERNMENT_ID",
    label: "Government ID",
  ...
  },
  {
    ...
    description: "Document proving accreditation status",
    required: false,
    acceptedTypes: ["application/pdf", "image/jpeg", "image/png"],
    maxSize: 10 * 1024 * 1024,
  },
...
function VerificationStatusPanel({
  ...
  const totalRequired = investorDocumentTypes.filter((d) => d.required).length;
  const uploadedRequired = documents.filter(
    (d) =>
      investorDocumentTypes.some(
        (docType) =>
  ...
  const statusConfig = {
    PENDING: {
      label: "Not Started",
      description: "Submit your investor documents to begin verification",
      color: "text-lime-400",
      progress: 0,
    },
    SUBMITTED: {
...
function EditableInvestorInfoField({
  field,
  value,
  onChange,
}: {
...
function InvestorInfoFormSection({
  profile,
  formData,
  onFieldChange,
  onSaveDraft,
  ...
  const fields = investorInfoFields;
...
export function InvestorKYCForm({
  ...
  useEffect(() => {
    ...
    investorInfoFields.forEach((field) => {
  ...
  const handleSaveDraft = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      const result = await saveInvestorDraft({
        walletAddress,
  ...
  const handleFileSelect = useCallback(async (docType: string, file: File) => {
    ...
    try {
      const docConfig = investorDocumentTypes.find((d) => d.type === docType);
      ...
      formDataForUpload.append("role", "investor");
  ...
  return (
    <>
      ...
      <section className="parallax-section mx-auto max-w-7xl px-5 pb-20">
        <div className="flex items-center justify-between gap-6 mb-6">
          <div>
            ...
            <p className="mt-2 text-white/64">
              Upload the required documents to verify your investor identity.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {investorDocumentTypes.map((docType) => {
            ...
            <div>
              ...
              <p className="mt-2 text-white/64">
                ...
                !profile?.verificationDocuments?.length
                  ...
                  : "Your investor verification has been verified and you have full access."}
              </p>
            </div>
            {(profile?.verificationStatus === "PENDING" ||
              profile?.verificationStatus === "REJECTED") && (
              <Link
                href="/verification/investor/submit"
        ...
        <BorderGlow className="glass-panel mx-auto max-w-7xl overflow-hidden p-7 sm:p-10 lg:p-14">
          <footer className="flex flex-col gap-4 border-t border-white/10 pt-6 text-sm text-white/46 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex gap-5">
              ...
              <Link href="/dashboard/investor" className="hover:text-white">
                Dashboard
              </Link>
            </div>
          </footer>
