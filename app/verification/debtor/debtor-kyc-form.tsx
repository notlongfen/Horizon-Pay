...
import { saveDebtorDraft } from "@/lib/verification/verification-service";
...
function VerificationStatusPanel({
  ...
  const totalRequired = debtorDocumentTypes.filter((d) => d.required).length;
  const uploadedRequired = documents.filter(
    (d) =>
      debtorDocumentTypes.some(
        (docType) =>
  ...
  const statusConfig = {
    PENDING: {
      label: "Not Started",
      description: "Submit your debtor documents to begin verification",
      color: "text-lime-400",
      progress: 0,
    },
    SUBMITTED: {
    ...
    VERIFIED: {
      label: "Verified",
      description: "Your debtor verification is complete",
...
function EditableDebtorInfoField({
  field,
  value,
  onChange,
}: {
...
function DebtorInfoFormSection({
  profile,
  formData,
  onFieldChange,
  onSaveDraft,
...
export function DebtorKYCForm({
  ...
  const handleSaveDraft = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      const result = await saveDebtorDraft({
        walletAddress,
        debtorInfo: formData,
      });
  ...
  const handleFileSelect = useCallback(async (docType: string, file: File) => {
    ...
    try {
      const docConfig = debtorDocumentTypes.find((d) => d.type === docType);
  ...
  return (
    <>
      ...
      <section className="parallax-section mx-auto max-w-7xl px-5 pb-20">
        <div className="flex items-center justify-between gap-6 mb-6">
          <div>
            ...
            <h2 className="text-2xl font-semibold text-white">
              Debtor Verification Documents
            </h2>
            <p className="mt-2 text-white/64">
              Upload the required documents to verify your debtor identity.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {debtorDocumentTypes.map((docType) => {
            ...
            <div>
              ...
              <p className="mt-2 text-white/64">
                ...
                !profile?.verificationDocuments?.length
                  ...
                  : "Your debtor verification has been verified and you have full access."}
              </p>
            </div>
            {(profile?.verificationStatus === "PENDING" ||
              profile?.verificationStatus === "REJECTED") && (
              <Link
                href="/verification/debtor/submit"
